...
KNOB<string> KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "MyPinTool.out", "specify profile file name");
KNOB<BOOL> KnobProfilePredicated(KNOB_MODE_WRITEONCE, "pintool", "p", "0", "enable accurate profiling for predicated instructions");
KNOB<BOOL> KnobProfileStaticOnly(KNOB_MODE_WRITEONCE, "pintool", "s", "0", "terminate after collection of static profile for main image");
...
VOID ComputeGlobalStats() {
  ...
  for (list<const BBLSTATS *>::iterator bi = statsList.begin(); bi != statsList.end(); bi++) {
    ...
    *out << StringFromAddrint((*bi)->_bbl_addr) << endl;
    for (UINT32 i = 0; i < MAX_REG; i++) {
      if (blockStats->reg_w[i] == 0 && blockStats->reg_r[i] == 0) continue;
      *out << decstr(i, 3) << " " << ljstr(REG_StringShort(REG(i)), 15) << decstr(blockStats->reg_r[i], 12) << decstr(blockStats->reg_w[i], 12) << endl;
    }
  }
}
...
VOID Fini(int, VOID *v) {
  ...
  *out << "#\n"
  ...
  for (UINT32 i = 0; i < MAX_REG; i++) {
    ...
    *out << decstr(i, 3) << " " << ljstr(REG_StringShort(REG(i)), 15) << decstr(GlobalStats.reg_r[i], 12) << decstr(GlobalStats.reg_w[i], 12) << endl;
  }
